...
import { BookOpen, Calendar, ChevronLeft, Eye, MessageCircle, MoreVertical } from 'lucide-react';
...
function UserDetails({userId}) {
  ...
  const {subContent, user} = useSelector((state) => state.adminContent)

  useEffect(() => {
    const fetchBlogs = async () => {
      try {
        const response = await axiosInstance.get(`blog/admin/get-user-blogs/${userId}`);
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
        <div className='flex items-center gap-1 p-3 hover:text-primary w-min'
        onClick={() => dispatch(setContent({subContent:null}))}
        >
    <ChevronLeft />
    back
</div>
    ...
    <div className="flex items-center justify-between">
      <div>
        <div className="flex items-center gap-4 mb-4">
          ...
          <h1 className="text-3xl font-bold text-gray-800">{user.username} Published Blogs</h1>
        </div>
        <p className="text-gray-600">
          Track the blogs {user.username}'ve published on Spotlight.
        </p>
      </div>
